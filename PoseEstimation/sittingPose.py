...
while True:
    ...
    for i, keypoints in enumerate(keypoints_data):
        ...
        if keypoints.shape[0] > 0:
            ...
            angle = calculate_angle(keypoints[11][:2], keypoints[13][:2],
                                    keypoints[15][:2])  # Kafa, kalça ve dizler arasındaki açıları aldık
            print(
                f"Kişi {i + 1} {'Sitting' if angle is not None and angle < 110 else 'Standing'} (Açı: {angle:.2f} derece)")
